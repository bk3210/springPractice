...
// DTO 객체에 담긴 정보를 가져옴
int totalCnt = Integer.parseInt(String.valueOf(request.getAttribute("totalCnt")));	// 총 페이지 갯수
%>
  ...
  <p>This example adds a dropdown menu in the navbar.</p>
   ...
   <colgroup>
    ...
    <%for(int i=0; i<list.size(); i++){
      ...
      <tr>
        <td><%=dto.getSeq() %></td>
        <td><a href="<%=request.getContextPath()%>/guestbook/view?seq=<%=dto.getSeq()%>"><%=dto.getTitle() %></a></td>
        <td><%=dto.getWriter() %></td>
        <td><%=dto.getWdate() %></td>
        <td><%=dto.getHit() %></td>
      </tr>
